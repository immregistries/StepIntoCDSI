...
The unit of work is one entry in `mappings/spec-to-code.yaml` (Phase 8) - a numbered spec section with a step package, or an `unmapped_classes` entry. There are 34 numbered units for version 4.6, all of which have a completed Role A pass as of 2026-09-07; two further `unmapped_classes` entries (`EVALUATE_GENDER`, `FORECAST_DATES_AND_REASONS`) exist but are deliberately excluded from Role A test-writing - see the Revision history entry below. Each unit is worked in two separate passes, run as separate agent sessions on the project owner's own schedule (not automated, not batched):
...
## Phase 22: Separate Static Supporting Data From Per-Run Forecast State

`DataModelLoader.createDataModel(supportingDataSet)` allocates a fresh `DataModel` and does a full, unbuffered re-parse of the selected Supporting Data ZIP on every call, with no caching anywhere in the path. `cdsi-fits-tests`' `FitsFixtureTest` calls it once per dynamic JUnit test - once per FITS fixture, 4,896 times in a full run - so the same static schedule data is reparsed thousands of times per suite run, and a full FITS run takes roughly 15-16 minutes. Phase 21's Role B workflow (`cdsi-engine/AGENTS.md`) requires the full FITS suite to be run, and compared for regressions, before and after every single fix; paying that cost repeatedly across many Role B rounds is a real drag on throughput. This phase closes that gap directly, and does so before Phase 21's Role B rounds begin in earnest rather than after, for a second, more important reason than speed: `DataModel` currently mixes static Supporting Data (the CVX map, antigen map/list, vaccine-group map/list, antigen series list, schedule list, live-virus conflict list, observation map) with per-request mutable state (forecast input, patient, immunization history, steppers, forecasts, loop guards, the current logic step, and other traversal fields) with no clean boundary between the two. Any future "cache and reuse one `DataModel`" shortcut - the fast but fragile path - would depend on an exhaustive, hand-maintained list of fields to reset between cases, a list Phase 21's own Role B fixes will keep changing; a missed field would let one FITS case's state leak into the next and silently corrupt the regression signal every later phase depends on. Establishing the boundary now, before more fields accumulate in the wrong place, is materially safer than retrofitting it later.

The target design is a `SupportingDataModel` holding exactly the static knowledge base, loaded once per selected Supporting Data set and treated as read-only for the duration of every run built from it, paired with a `DataModel` that owns only one forecast run's mutable working state and references the shared `SupportingDataModel` for everything else:

1. Introduce `SupportingDataModel` in `cdsi-engine`, holding the CVX map, antigen map/list, vaccine-group map/list, antigen series list, schedule list, live-virus conflict list, observation map, and any further Supporting-Data-owned structures Phase 21's Role B rounds add going forward (association-age metadata, equivalent series groups, conditional-skip context, immunity/contraindication definitions) - so those land directly in the right place instead of back into `DataModel`.
2. Reduce `DataModel` to per-run mutable state only: forecast input, patient, immunization history, assessment date, current/previous logic step, current antigen/vaccine group/target dose/forecast, selected lists and position counters, the patient series stepper, the selected/scorable/prioritized/best patient-series lists, forecast lists, loop guards and orchestration state, and a reference to the shared `SupportingDataModel`.
3. Make `DataModelLoader.createDataModel(supportingDataSet)` the single entry point: load (or reuse a cached) `SupportingDataModel` once per distinct Supporting Data set per run, then construct a fresh `DataModel` per case or request that references it. Keep `DataModel`'s existing supporting-data getters as thin delegates onto the shared `SupportingDataModel` so most call sites in the ~30 step classes do not need to change as part of this phase.
4. Treat everything reachable from `SupportingDataModel` as read-only once loaded, even though the existing domain classes were not built as immutable - prefer unmodifiable collection wrappers where practical, keep mutating setters loader-only or package-private where practical, and explicitly document any shared mutable object that is intentionally treated as read-only rather than attempting a full immutability rewrite of the domain model in this phase.
5. Add regression tests that fingerprint a loaded `SupportingDataModel` before and after a full run and assert it is unchanged, and that run two very different FITS cases back to back in both orders and assert identical results by case id - proof of no cross-case contamination that does not depend on execution order.
6. Update `FitsEngineRunner` (and the servlet path in `cdsi-web`, if `FitsServlet.runTestsForGroup(...)` has the same reload-per-request shape) to load Supporting Data once per suite run and hand out a fresh `DataModel` per case, instead of reparsing the ZIP per case.
7. Run the full `cdsi-engine` suite and the full FITS suite before and after, and compare pass/fail/error counts and `known-passing-cases.txt` membership exactly. This phase's acceptance bar is zero functional change - any difference other than runtime is a regression to explain and fix, not progress to report.
8. Record full-suite FITS runtime before and after alongside the pass/fail comparison, so the performance benefit this phase exists to capture is on the record rather than assumed.

The agent must not:

- Reuse a single mutable `DataModel` instance across cases or requests ("reset and reuse") as the primary strategy. An exhaustive, ever-changing field-clearing list is more fragile than a fresh instance plus a shared read-only `SupportingDataModel`, and it is exactly the shortcut this phase exists to avoid.
- Change any clinical logic, decision-table behavior, date calculation, or Supporting Data interpretation as part of this phase. It is a pure non-functional refactor; a discrepancy noticed along the way is recorded as a Phase 9 finding for a separate, later, reviewed fix - never corrected inline here.
- Treat an improved FITS pass rate as evidence this phase succeeded. The target result is identical FITS pass/fail/error counts and identical `known-passing-cases.txt` membership, not a higher pass count.
- Turn on broad parallel test execution before this boundary exists and is proven safe by the case-order-independence tests in step 5. Parallelism can hide cross-case mutation bugs instead of revealing them.
- Deep-copy the entire Supporting Data graph per case as a shortcut to avoid addressing mutability. That would erase the performance benefit this phase exists to capture; use it only selectively, for a specific object that genuinely cannot be made safely shareable.

This phase formalizes and supersedes three outside advisory notes written for this purpose: `docs/26-fits-suite-performance-review.md` (the performance diagnosis and the `SupportingDataModel`/`DataModel` design), `docs/27-reference-implementation-stewardship-pass.md` (which names this split as the one piece of its later cleanup/alignment phase that should happen early, before Phase B logic repairs rather than after), and `docs/28-phase-b-fits-progress-ledger.md` (which proposes this phase as the first proving case for a reviewed before/after progress ledger - zero expected functional delta, a large expected runtime delta). Building that ledger is not required to complete this phase; the plain full-suite pass/fail/runtime comparison in step 7-8 above is sufficient, and the ledger tooling itself is Phase 23, a separate later effort.

**Capture the before/after story durably, not just in a terminal.** Because Phase 23 (below) will want this phase's own before/after as its first real entry, step 7-8's comparison must be written to a durable, committed record before this phase is considered done - not only reported in a chat transcript or left in disposable `target/fits-runs/<run-id>/` bundles. At minimum that record needs: exact case-level counts (passed/failed-assertion/execution-error, not just the surefire-level allowlist pass/fail), the full-suite runtime before and after, the `reference-set.json`/`run.json` ids each side was measured against, and a short narrative of what was done and why - the same level of detail this plan's own Revision history entries carry, since a future reader deciding whether to trust this as Phase 23's seed data needs the story, not only the two headline numbers.

## Phase 23: Build the Phase B Progress Story

Once Phase 22 lands and is confirmed to change no results, the project has its first real before/after data point for the reviewed progress ledger `docs/28-phase-b-fits-progress-ledger.md` proposed - but that note, read closely, is written for one audience: the project owner and agents deciding whether a Phase B round was safe to keep. It is a dense, exact, reviewer-facing design (before/after counts, group deltas, newly-passing/newly-failing case lists). There is a second, related but genuinely different audience this project also needs to serve: people who were not in the weeds of any of this and want to see, in a couple of minutes, that the project is real and moving - a before/after story, a headline number or two, a trend, with the engineering detail available one click deeper but not required reading. Phase 23 builds both from one shared source of truth rather than building one artifact and hoping it serves both jobs.

This phase formalizes and supersedes `docs/28-phase-b-fits-progress-ledger.md`, extending it with the second audience:

1. **The reviewed ledger** (doc 28's own design, largely as written): a committed, structured before/after record per reviewed Phase B round - baseline run id/commit, after run id/commit, pass/fail/error counts on both sides, full-suite runtime on both sides, group-level deltas, newly-passing and newly-failing case lists (or a link to where they're recorded), related finding ids, and a short reviewer-facing explanation of what changed and why. Phase 22's own before/after (captured durably per that phase's own instruction above) becomes this ledger's first entry - back-filled from the record Phase 22 already leaves behind, not re-measured.
2. **The story view** - a second, much simpler rendering built from the same entries, meant to be shown to someone with no context: what the project is, where it started, where it is now, and that concrete, verified progress is being made - headline numbers (FITS pass rate, spec-conformance test coverage from Phase 21's dashboard, a runtime figure like Phase 22's), a short timeline of reviewed rounds, and a link into the reviewed ledger for anyone who wants the detail. This view must never overstate status - every number it shows must trace back to a reviewed ledger entry, never to an in-progress or unreviewed run.

Building this after Phase 22 rather than before is deliberate: a ledger designed only from Phase 22's zero-functional-delta round risks being shaped around a case that has no interesting deltas to show; waiting until at least one real clinical-logic Phase B round exists (once Role B sequencing, per Phase 21's Cross-cutting risk section, picks a first round) would give the design a harder case to prove itself against. Whether to wait for that or proceed with Phase 22 alone as the seed is the project owner's call when this phase actually starts - not decided here.

The agent must not:

- Build the story view before the reviewed ledger exists for at least the rounds it displays - the story is a rendering of reviewed ledger entries, never an independent source of numbers.
- Let the story view's simplicity drop context that changes its meaning - "same FITS results, five times faster" is a fair headline for Phase 22; a future round that improves FITS pass rate at the cost of an unresolved regression elsewhere must show that caveat, not hide it for a cleaner story.
- Treat this phase as blocking Role B - per Phase 21's own Non-Goals, FITS pass-rate movement is not that phase's goal, and this phase's dashboard is a reporting layer on top of reviewed work, not a gate in front of it.
...
23. Introduce `SupportingDataModel`, reduce `DataModel` to per-run state, and add the case-order-independence and unmutated-Supporting-Data regression tests.
24. Add the reviewed progress ledger and the story view built on top of it, seeded with Phase 22's own before/after entry.
...
- `SupportingDataModel` is loaded once per selected Supporting Data set per suite run, and every FITS case or forecast request receives a fresh, isolated `DataModel` that references it - never a reused, hand-reset instance.
- `cdsi-engine` and FITS pass/fail/error counts, and `known-passing-cases.txt` membership, are identical before and after the `SupportingDataModel`/`DataModel` split, with the measured full-suite runtime reduction recorded alongside that comparison.
- Tests demonstrate that running two different FITS cases back to back in both orders produces identical results, and that a loaded `SupportingDataModel` is unmutated after a run - not merely that no contamination has been observed.
- Every reviewed Phase B round, starting with Phase 22 itself, leaves a durable, committed before/after record - counts, runtime, and a short narrative - before being considered done, not only a chat transcript or a disposable run bundle.
- The story view (Phase 23) never displays a number that doesn't trace back to a reviewed ledger entry, and never runs ahead of what the reviewed ledger actually contains.
...
- Changing any clinical logic, decision-table behavior, date calculation, or Supporting Data interpretation as part of Phase 22's `SupportingDataModel`/`DataModel` split - it is a pure non-functional refactor with an identical-results acceptance bar, not an opportunity to fix a defect found along the way.
- Reusing a single mutable `DataModel` across cases as a Phase 22 performance shortcut ("reset and reuse"). A fresh `DataModel` per case referencing a shared, read-only `SupportingDataModel` is the required shape, not an exhaustive hand-maintained field-clearing list.
...
- Phase 21's Role A pass completed all 34 numbered units (4.1 through 9.3) on 2026-09-07. The project owner decided not to extend Role A to the two `unmapped_classes` entries (`EVALUATE_GENDER`, `FORECAST_DATES_AND_REASONS`): both are expected to be removed or reclassified as non-spec infrastructure under the disposition guidance already recorded in their `spec-to-code.yaml` notes (added 2026-09-05), so writing spec-conformance tests for logic likely headed for deletion isn't worth the effort. `step-tests/status.yaml` records both as `not_started` with a note explaining this is a deliberate scope decision, not a gap - the dashboards' "34/36" figure reflects that decision rather than remaining work. Role B sequencing (see the Cross-cutting risk section above) can now proceed with a complete evidence base across every numbered unit.
- On 2026-09-07, immediately after Phase 21's Role A pass completed, Phase 22 (Separate Static Supporting Data From Per-Run Forecast State) was added to formalize three outside advisory notes (`docs/26-fits-suite-performance-review.md`, `docs/27-reference-implementation-stewardship-pass.md`, `docs/28-phase-b-fits-progress-ledger.md`) into a numbered phase with its own acceptance criteria, ahead of Phase 21's Role B rounds and the wider Phase B clinical-logic repair effort those advisory notes anticipate. The project owner chose to do this work next, before any Role B fixing begins, for two reasons: it cuts the full FITS suite's ~15-16 minute runtime (paid on every Role B round's mandatory before/after regression check) toward roughly a minute by ending the current per-case Supporting Data ZIP re-parse, and - the more important reason - it establishes an explicit static/mutable boundary in `DataModel` before Phase 21's Role B fixes add more fields to the area that boundary needs to separate. An earlier draft of this numbering scheme used "Phase 22" for a different, later-removed idea (Generate Historical Charts - see the first entry in this history); that phase was fully retired well before this one was added; the number was not otherwise reserved.
- Phase 22 was completed the same day (2026-09-07). Full before/after record in `docs/29-phase-22-before-after.md`: the full `cdsi-engine` suite (764 tests, 192 failures, 1 error) and the full FITS suite (4,896 cases: 3,364 passed, 1,531 failed-assertion, 1 execution-error) are byte-for-byte identical before and after, and full FITS suite runtime dropped from 457.5s to 39.9s (11.5x). Two things surfaced along the way, both recorded rather than smoothed over: establishing the baseline first required fixing an unrelated, pre-existing FITS fixture line-ending drift that was blocking every FITS run outright (`371decb`), and the first "after" measurement showed no speedup at all because `cdsi-fits-tests` was silently still resolving the old, pre-split `cdsi-engine` jar from the local Maven repository rather than the rebuilt one - caught and corrected (`mvn install`) before trusting the result. Role B sequencing (Phase 21's Cross-cutting risk section) can now proceed against a materially faster FITS regression loop.
- Phase 23's reviewed-ledger half was built the same day (2026-09-07), before Role B sequencing rather than after, at the project owner's request - a visible "here's where we started, here's each change, here's where we are now" was wanted immediately, not deferred until several Role B rounds had already happened undocumented. Added `cdsi-reference/progress-ledger/entries/<id>.yaml` (one immutable file per reviewed round, schema at `schemas/progress-ledger-entry.schema.json`, mirroring `reference-sets/<id>.yaml`'s convention exactly), the `progress_ledger.py` module and `progress-ledger list`/`validate`/`dashboard` CLI commands (mirroring `fits_dashboard.py`/`step_test_dashboard.py`'s exact code and CSS conventions), a third tile on `dashboards/index.html`, and one backfilled entry transcribing Phase 22's own already-verified `docs/29` numbers - no new measurement, a structured restatement of an already-committed record. The second half of Phase 23 (a simplified view for a non-technical audience, built on top of the same reviewed entries) remains deferred, per the project owner's own "don't need to build now, but don't lose the story" instruction from when Phase 23 was first added.
- Also on 2026-09-07, while Phase 22's before/after baseline was being captured, Phase 23 (Build the Phase B Progress Story) was added, formalizing and extending `docs/28-phase-b-fits-progress-ledger.md`. The project owner's own framing drove the change: `docs/28` was written for one audience (the project owner and agents deciding whether a Phase B round is safe to keep), but the project also needs to show real, verified progress to people outside that loop - not instead of the reviewed ledger, but as a second, simpler rendering built from the same reviewed entries, never an independent source of numbers. Scoped to start after Phase 22 completes, seeded with Phase 22's own before/after as its first entry - which is also why Phase 22 itself picked up a new requirement to capture that before/after as a durable, committed record rather than only reporting it in a session.
