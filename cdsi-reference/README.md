...
| `step-tests/dashboard.html` | Fully generated by `step-tests dashboard`; gitignored - a disposable local view, safe to delete and regenerate, never hand-edited |
...
`step-tests dashboard` renders the same information as a single self-contained, gitignored static HTML file (`step-tests/dashboard.html` by default) - progress totals with bars, blocked units surfaced first, and a filterable/sortable table, all inline CSS/JS with no external dependencies or network access, meant to be opened directly in a browser and regenerated on demand (after `mvn -pl cdsi-engine test`) rather than served or committed.
