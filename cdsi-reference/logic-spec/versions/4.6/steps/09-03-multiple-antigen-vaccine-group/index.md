...
**[IMPLEMENTATION]** `MULTIANTVG_1()` computes both contained-date aggregates first, then branches once: if any contained forecast is a FORECASTPRIORITY-1 priority forecast, the group earliest date is the later of the earliest contained earliest date and the latest date administered of a dose belonging to the vaccine group; otherwise it is the latest earliest date of the contained forecasts (SPEC-4.6-0049). "Belonging to the vaccine group" is CVX-to-antigen overlap against the group's antigen list (SPEC-4.6-0061). Schedule Supporting Data never populates `VaccineGroup.vaccineList`, so catalog identity matching alone left the last-administered floor null in production.
...
- **MULTIANTVG-1 last-administered floor** (SPEC-4.6-0049 wrote the clause; SPEC-4.6-0061 made it see production doses): membership is CVX-to-antigen overlap against the vaccine group's antigens, because Schedule Supporting Data never fills `VaccineGroup.vaccineList`.
