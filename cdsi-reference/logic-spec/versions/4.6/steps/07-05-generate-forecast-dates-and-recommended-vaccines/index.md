...
- **Documented fix (2026-09-16, SPEC-4.6-0065): CALCDTLIVE-4 from the full immunization history.** Table 7-12's latest conflict-end date is CALCDTCONFLICT-2 over previous live-virus doses whose type conflicts with a preferable vaccine type of the target being forecast, not the current antigen's previous AAR. Valid/unevaluated previous doses use `minConflictEndInterval`; otherwise `conflictEndInterval`. A Varicella Dose 1 forecast after MMR now floors at MMR+28d (`VAR-2013-0840`); a Not Valid too-young Varicella floors at that shot+28d (`VAR-2013-0803`). 7.4 still omits this candidate from its own FORECASTDTCAN-1.
