...
- **Documented fix (2026-09-15, SPEC-4.6-0056): §3.3 Age selection for evaluation.** Table 6-14's calculated age dates must come from the Age row whose Effective–Cessation window covers the date administered (RELEVANT-1), not `getAgeList().get(0)`. `RelevantSupportingData.selectAge` is the same helper 7.5 uses with the assessment date. Polio Dose 4's ceased 18-week row no longer evaluates modern administrations. Forecast-path leftover in 7.4 and preferable-interval selection in 6.5 are the rest of SPEC-4.6-0056.
