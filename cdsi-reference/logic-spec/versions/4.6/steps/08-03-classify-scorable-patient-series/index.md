...
- **Possible gap in Table 8-5's partition:** a series group with, say, exactly 1 complete series and 1 in-process series (and not "0 valid doses for all") matches none of the three rules as literally written (rule 1 needs 2+ complete, rule 2 needs 2+ in-process AND 0 complete). The implementation's default outcome prefers complete scoring when any complete series is present (SPECIFICATION_AMBIGUITY). 8.4 then drops the in-process siblings (SPEC-4.6-0064).
