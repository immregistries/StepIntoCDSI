...
- **Documented fix (2026-09-15, SPEC-4.6-0056): §3.3 Age/Preferable Interval selection for forecast-need.** CALCDTAGE-1 and FORECASTDTCAN-1 now read the Age and Preferable Interval rows whose Effective–Cessation window covers the assessment date (RELEVANT-2), matching 7.5. Previously `getAgeList().get(0)` and the full interval list were used. Completes the forecast-path leftover from SPEC-4.6-0055.
- **Documented deviation (2026-09-15): assessment-relative seasonal recommendation dates.** Table 7-9 / Table 7-11 and FORECASTDTCAN-1 treat Supporting Data `<seasonalRecommendation><startDate>/<endDate>` as literal absolute dates. CDC releases those windows one season at a time, so a bundled release (e.g. Influenza 2025-07-01 through 2026-06-30) is the wrong calendar year for FITS fixtures assessed in 2020/2024, and goes stale for live assessments once that season ends while the next CDC file has not landed. This engine keeps the loaded window as a *template* and projects it by whole years around the assessment date via `SeasonalRecommendationDates` (used here for the seasonal end date and FORECASTDTCAN-1's seasonal start candidate). Inter-season gaps (e.g. Sep–Mar windows with a summer assessment) use the *upcoming* season so the next start date can still be forecast. Under that projection, Table 7-11 Rule 6 ("past seasonal recommendation end date") is effectively unreachable for normal annual templates - a deliberate tradeoff for CDC-lag / multi-year FITS support. See SPEC-4.6-0051.
...
- **FORECASTDTCAN-1's implementation (`computeEarliestDate()`) appears to omit two of the six candidate dates the rule specifies** - the latest conflict-end-interval date and the seasonal recommendation start date - based on commented-out lines in the source (one with a self-doubting comment, `// CALCDTLIVE-4 is both used and removed?`). Not confirmed as a defect (the commented code might be genuinely superseded elsewhere), but flagged as unverified rather than assumed complete - would need someone to trace whether those two dates are folded in via a different code path (e.g. within 6.7's conflict-interval computation) before this can be called a real gap. *(2026-09-15 note: seasonal start is included via `SeasonalRecommendationDates.effectiveStartDate`; the conflict-end omission note is unchanged.)*
