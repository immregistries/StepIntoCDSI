...
- **Known-passing allowlist** (`src/test/resources/known-passing-cases.txt`): a case listed here is asserted - if it starts failing, the build fails, because that's a real regression. Every other case still runs and is still fully recorded in the diagnostic bundle, but is only checked with a JUnit assumption, so it reports as skipped rather than failing the build. This means "the build is green" only ever means "every case we've verified we can pass, we still pass," never "we're making progress toward passing everything" - see `README.md`'s "Known-passing allowlist" section for exactly how and when to add a case to it.
...
11. **Stop for the project owner's review before committing a clinical-logic change.** Propose the finding and the fix together; do not commit or merge a `cdsi-engine`/`cdsi-web` change unilaterally, no matter how many tests pass locally. Recording a finding, or adding a case id to an existing one, doesn't need that same review gate - it's documentation, not a behavior change. Regenerating `known-passing-cases.txt` to add newly-fixed cases is part of that same reviewed change, never a separate unilateral step (see `README.md`'s "Regenerating the allowlist").
