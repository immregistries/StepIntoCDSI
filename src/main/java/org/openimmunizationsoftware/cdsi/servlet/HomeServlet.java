...
public class HomeServlet extends HttpServlet {
        ...
        protected void doGet(HttpServletRequest req, HttpServletResponse resp)
                ...
                // Get selected supporting data set from request parameter, or use default
                String selectedSupportingDataSet = req.getParameter("supportingDataSet");
                if (selectedSupportingDataSet == null || selectedSupportingDataSet.trim().isEmpty()) {
                        selectedSupportingDataSet = SupportingDataManager
                                        .resolveDefaultSupportingDataSet(getServletContext());
                } else {
                        selectedSupportingDataSet = SupportingDataManager.normalizeSetId(selectedSupportingDataSet);
                }

                // Get list of available supporting data sets
                java.util.List<SupportingDataManager.SupportingDataDescriptor> supportingDataDescriptors = SupportingDataManager
                                .listSupportingDataDescriptors(getServletContext());

                // Get descriptor for selected supporting data set
                SupportingDataManager.SupportingDataDescriptor selectedDescriptor = SupportingDataManager
                                .findSupportingDataDescriptor(getServletContext(), selectedSupportingDataSet);
                ...
                out.println("    .supporting-data-selector { margin: 15px 0; }");
                out.println("    .supporting-data-selector label { font-weight: bold; margin-right: 10px; }");
                out.println("    .supporting-data-selector select { padding: 5px; font-size: 14px; }");
                out.println("  </style>");
                out.println("  <script>");
                out.println("    function changeSupportingData() {");
                out.println("      var select = document.getElementById('supportingDataSelect');");
                out.println("      var selectedValue = select.value;");
                out.println("      window.location.href = '?supportingDataSet=' + encodeURIComponent(selectedValue);");
                out.println("    }");
                out.println("  </script>");
                out.println("</head>");
                out.println("<body>");
                ...
                // Supporting data selector dropdown
                out.println("  <div class=\"supporting-data-selector\">");
                out.println("    <label for=\"supportingDataSelect\">Supporting Data:</label>");
                out.println("    <select id=\"supportingDataSelect\" onchange=\"changeSupportingData()\">");
                for (SupportingDataManager.SupportingDataDescriptor descriptor : supportingDataDescriptors) {
                        String displayName = descriptor.knowledgeBaseId + " v" + descriptor.version + " ("
                                        + descriptor.zipName + ")";
                        boolean isSelected = descriptor.setId.equals(selectedSupportingDataSet);
                        out.println("      <option value=\"" + escapeHtml(descriptor.setId) + "\""
                                        + (isSelected ? " selected" : "") + ">" + escapeHtml(displayName)
                                        + "</option>");
                }
                out.println("    </select>");
                out.println("  </div>");
                ...
                for (SandboxServletExamples.ExampleCategory category : examples) {
                        for (SandboxServletExamples.ExampleLink link : category.links) {
                                // Build URLs with supporting data parameters
                                String baseUrl = escapeHtml(link.url);
                                String stepUrl = "/step/step" + baseUrl + "&supportingDataSet="
                                                + escapeHtml(selectedSupportingDataSet);
                                String forecastUrl = "/step/forecast" + baseUrl + "&supportingDataSet="
                                                + escapeHtml(selectedSupportingDataSet);
                                String runUrl = "/step/run" + baseUrl + "&supportingDataSet="
                                                + escapeHtml(selectedSupportingDataSet);

                                // For FHIR Sandbox, add knowledgeBase and knowledgeBaseVersion parameters
                                String sandboxUrl = "/step/sandbox" + baseUrl;
                                if (selectedDescriptor != null) {
                                        sandboxUrl += "&knowledgeBase="
                                                        + escapeHtml(selectedDescriptor.knowledgeBaseId);
                                        sandboxUrl += "&knowledgeBaseVersion=" + escapeHtml(selectedDescriptor.version);
                                }
                                ...
                                out.println("        <a href=\"" + stepUrl + "\">Step</a>");
                                out.println("        <a href=\"" + forecastUrl + "\">Forecast</a>");
                                out.println("        <a href=\"" + runUrl + "\">Run</a>");
                                out.println("        <a href=\"" + sandboxUrl + "\">FHIR Sandbox</a>");
                                out.println("      </td>");
                                out.println("    </tr>");
                        }
                }
