...
public class EvaluateLiveVirusConflict extends LogicStep
{
  ...
  public EvaluateLiveVirusConflict(DataModel dataModel)
  {
    ...
    caCurrentVaccineType = new ConditionAttribute<VaccineType>("Supporting Data (Live Virus Conflict)" , "Current Vaccine Type");
    caPreviousVaccineType = new ConditionAttribute<VaccineType>("Supporting Data (Live Virus Conflict)" ,  "Previous Vaccine Type");
  ...
  private class LT420 extends LogicTable
  {
    public LT420() {
    	...
    	setLogicCondition(0, new LogicCondition("Is the current vaccine type of the vaccine dose administered one of the supporting data defined live virus conflict current vaccine types?"){
			@Override
    	...
    	setLogicCondition(1, new LogicCondition("Is there at least one vaccine dose administered on or before the current vaccine dose administered date?") {//CHeck
    	...
    	setLogicResults(0, YES,YES);
    	setLogicResults(1, NO,ANY);
    	setLogicResults(2, ANY,NO);
...
private class LT421 extends LogicTable{
	...
	public LT421() {
		...
		setLogicCondition(0, new LogicCondition("Is the vaccine type of the previous vaccine dose administered the same as one of the supporting data defined live virus conflict previous vaccine types when the current vaccine dose administered type is same as the live virus conflict current vaccine type ?") {	
			@Override
			protected LogicResult evaluateInternal() {
...
private class LT422 extends LogicTable{
	...
	public LT422() {
		super(1, 2,"Table 4-22 Is the current vaccine dose administrated in conflict with previous vaccije dose administrated ?");
		
		setLogicCondition(0, new LogicCondition("Is the conflict begin interval date ≤ current date administered<conflict end interval date?") {
			
			@Override
			protected LogicResult evaluateInternal() {
				for(LiveVirusConflict lvc: dataModel.getLiveVirusConflictList()){
					if(caDateAdministered.getFinalValue().after( caConflictBeginIntervalDate.getAssumedValue()) && caDateAdministered.getFinalValue().before(caConflictEndIntervalDate.getAssumedValue())){
						return YES;
					}
				}
