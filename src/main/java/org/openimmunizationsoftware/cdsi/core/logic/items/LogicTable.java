...
public class LogicTable {
  ...
  public void evaluate() throws IllegalStateException {
    ...
    for (int j = 0; j < logicOutcomes.length; j++) {
      ...
      // So the way we get this is a != b && a != c -> !(a ==b || a == c) (DeMorgans
      // Law)
      // By that logic we could go further and say since allTrue starts true we could
      // set it to false the start and then drop ! from !(a ==b Or a == c)
      // since !(a ==b Or a == c) = !d -> (a ==b Or a == c) = d
      // So we could set allTrue to true if it held for every value in out loop.

      // The disadvantage of this is we we would lose the ability to break for some
      // efficient gains as we could just throw a break after allTrue = false;
      // right now. Though i think the readability outweighs this.

      // We should also just have a method in LogicCondition that does this logic and
      // returns a boolean with some meaningful name so its much more readable.

      // So basically this ends up saying if EVERY LogicalOutcome in our column is
      // either ANY or the as the LogicResult of the condition
      // Then we can evaluate the column. Which we do if allTrue is true(really bad
      // name) I think we should call it something like canEvaluate or something
      // BTW if we this we could also then remove the next if condition and just run
      // perform here.
