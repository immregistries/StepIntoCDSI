...
public class OrganizeImmunizationHistory extends LogicStep
{
  ...
  public LogicStep process() {
    ...
    Collections.sort(dataModel.getAntigenAdministeredRecordList(), new Comparator<AntigenAdministeredRecord>() {
      @Override
  ...
  public void printPost(PrintWriter out) {
    ...
    for (VaccineDoseAdministered vda : dataModel.getImmunizationHistory().getVaccineDoseAdministeredList()) {
      out.println("  <tr>");
      out.println("    <td>" + vda.getVaccine().getTradeName() + " (" + vda.getVaccine().getVaccineType().getCvxCode()
          + "/" + vda.getVaccine().getManufacturer() + ") - " + vda.getVaccine().getVaccineType().getShortDescription()
          + "</td>");
