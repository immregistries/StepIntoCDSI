...
public class EvaluatePreferableInterval extends LogicStep {
  ...
  private void printStandard(PrintWriter out) {
    ...
    out.println(
        "<p>Figure 6-7 provides the evaluation interval timeline used to define adjacent intervals by using from immediate previous dose administered as the reference point.</p>");
    out.println("<img src=\"Figure 6.7.PNG\"/>");
    out.println("<p>FIGURE 6 - 7 EVALUATE INTERVAL 'FROM IMMEDIATE PREVIOUS DOSE ADMINISTERED FLAG' TIMELINE</p>");

    out.println(
      "<p>Figure 6-8 illustrates the evaluation interval timeline used to define non-adjacent intervals by using from target dose number in series as the reference point. This timeline is used only when from immediate previous dose administered flag is 'N'.</p>");
    out.println("<img src=\"Figure 6.8.png\"/>");
    out.println(
        "<p>FIGURE 6 - 8 EVALUATE INTERVAL 'FROM TARGET DOSE NUMBER IN SERIES' TIMELINE</p>");

    out.println(
        "<p>Figure 6-9 illustrates the evaluation interval timeline used to define most recent vaccine intervals.</p>");
    out.println("<img src=\"Figure 6.9.PNG\"/>");
    out.println(
        "<p>FIGURE 6 - 9 EVALUATE INTERVAL ‘FROM MOST RECENT VACCINE TYPE’ TIMELINE</p>");

    //TODO: Add figure 6-10 EVALUATE INTERVAL 'FROM RELEVANT OBSERVATION CODE' TIMELINE

    out.println(
        "<p>The following process model, attribute table, decision table, and business rule table are used to evaluate the preferable interval of a vaccine dose administered</p>");    
    out.println("<img src=\"Figure 6.11.png\"/>");
    out.println("<p>FIGURE 6 - 11 EVALUATE INTERVAL PROCESS MODEL</p>");
    ...
    if (seriesDose.getIntervalList().size() == 0) {
      ...
      //TODO: change this table to match 4.5
      out.println("<table>");
      out.println("  <tr>");
